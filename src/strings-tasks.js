 ...
 *   sumOfCodes('') => 0Arrow function should not return assignment
 *   sumOfCodes() => 0
 */
function sumOfCodes(str) {
  ...
  return str.split('').reduce((akk, item) => {
    return akk + item.charCodeAt();
  }, 0);
}
...
function countVowels(str) {
  const contains = str.toLowerCase().match(/[aeiouy]/g);
...
function encodeToRot13(str) {
  ...
  return str.split('').reduce((changedStr, item) => {
    return inp.includes(item)
      ? changedStr + out[inp.indexOf(item)]
      : changedStr + item;
  }, '');
}
